...
# set the txt file for read and write
map_file = "map.txt"
puzzle_file = "puzzle.txt"

# set the blank list use for saving 
room_order = []
sora_tower_map = []
sora_tower = None
...
class current_map:
    def __init__(self, map):
        """
        Define the object inside the current_map class
        """
        self.map = map
    ...
    def __getitem__(self, index, index1, index2):
        """
        Define the index needed for user to gain the item/item location
        """
...
def create_puzzle_order():
    """
    Create a random order for puzzle room
    """
    global room_order
...
def write_puzzle_order():
    """
    write down the random order for puzzle
    """
    global room_order
...
def read_puzzle_order():
    """
    Read the saved randown puzzle order
    """
    global room_order
    try:
...
def initial_map():
    """
    define the map that going to be use as room order, 
    also read and write the map out.
    """
    global sora_tower
